(() => {
  if (customElements.get("vocabulary-status-button")) {
    console.warn("[vocabulary-status-button.js] 组件已注册，跳过重复定义");
    return;
  }
  ...
  const func = (root, initStatus, changeStatus) => {
    ...
    let pointerHoverOffset = { x: 0, y: 0 };
    let buttonScale = 1;

    const clamp = (value, min, max) => Math.max(min, Math.min(max, value));

    const updateButtonTransform = () => {
      const totalX = basePosition.x.toFixed(4);
      const totalY = basePosition.y.toFixed(4);
      mainButton.style.transform = `translate(${totalX}em, ${totalY}em) translate(${pointerHoverOffset.x}px, ${pointerHoverOffset.y}px) scale(${buttonScale})`;
    };

    const setBasePosition = (x, y = 0) => {
      basePosition = { x, y };
      updateButtonTransform();
    };

    const setPointerHoverOffset = (xPx = 0, yPx = 0) => {
      pointerHoverOffset = { x: xPx, y: yPx };
      updateButtonTransform();
    };

    const setButtonScale = (scale = 1) => {
      buttonScale = scale;
      updateButtonTransform();
    };
    ...
    const handlePointerMove = (event) => {
      ...
      const offsetX = clamp(
        dx * POINTER_STRENGTH * influence,
        -POINTER_MAX_OFFSET,
        POINTER_MAX_OFFSET
      );
      const offsetY = clamp(
        dy * POINTER_STRENGTH * influence,
        -POINTER_MAX_OFFSET,
        POINTER_MAX_OFFSET
      );

      setPointerHoverOffset(offsetX, offsetY);
    };
    ...
    components.addEventListener("mousemove", handlePointerMove);
    components.addEventListener("mouseleave", handlePointerLeave);
